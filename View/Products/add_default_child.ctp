<div class="productAdd form">
    ...
    <fieldset>
		...
		foreach ($this->request->data['Option'] as $select) {
			...
			if (!empty($options)) {
				echo $this->Form->input('Option.Option.' . $i, array(
					'label' => __('%s <small>(%s)</small>', $select['name'], $this->Html->link('add new', '#', array('class' => 'newOption', 'data-target' => '#ChildOption' . $i . 'Name'))),
					'type' => 'select',
					'options' => $options,
					'id' => 'ChildOptionOption' . $i
				));
    ...
    <fieldset>
		...
		echo $this->Form->input('Product.product_brand_id', array(
			'empty' => '-- Select --',
			'label' => 'What is the brand name for this product? ('
						. $this->Html->link('add', array('controller' => 'product_brands', 'action' => 'add'))
						. ' / ' . $this->Html->link('edit', array('controller' => 'product_brands', 'action' => 'index'))
						. ' brands)'
		));
