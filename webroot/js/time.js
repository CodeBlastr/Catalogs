...
function displayTZCountDown(countdown,tzcd)  {
	...
	//alert(todate_extra);
	if (countdown < 0) 
		document.getElementById(tzcd).innerHTML = "Time is up.";
	...
	if (secs < 10) secs = '0'+secs;
		...
		if (mins < 10) mins = '0'+mins;
			...
			if(days > 1){
				...
				str	=	days + ' Days ' + (days == 1 ? '' : '') + hours + ':' + mins + ':' + secs;
			} else if(days == 1){
				str	=	days + ' Day ' + (days == 1 ? '' : '') + hours + ':' + mins + ':' + secs;
		} else {
			str	=	days + ' Days ' + (days == 1 ? '' : '') + hours + ':' + mins + ':' + secs + 's';
		}
